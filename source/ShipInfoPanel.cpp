...
void ShipInfoPanel::UpdateInfo()
{
	...
	info.Update(ship, player.FleetDepreciation(), player.GetDate().DaysSinceEpoch(), player.GetPlanet());
...
void ShipInfoPanel::Dump()
{
	...
	info.Update(**shipIt, player.FleetDepreciation(), player.GetDate().DaysSinceEpoch(), player.GetPlanet());
	if(loss)
...
void ShipInfoPanel::DumpPlunder(int count)
{
	...
	if(count > 0)
	{
		...
		info.Update(**shipIt, player.FleetDepreciation(), player.GetDate().DaysSinceEpoch(), nullptr);
